...
function find(p_drug_id drug.drug_id%type) return trec pipelined;
...
v_branch_id number default employee_pkg.get_branch_id(p_employee_id);
begin
...
(select drug_id,pharmacy_branch_id,count(*) cnt from sold_drugs  group by drug_id,pharmacy_branch_id ) sold  right join
(select drug_id,pharmacy_branch_id,count(*) cnt from available_drugs_store  group by pharmacy_branch_id,drug_id) distribute
...
function find(p_drug_id drug.drug_id%type) return trec  pipelined as
  begin
